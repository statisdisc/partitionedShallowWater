...
def stability_space(h0,h1,c0,Cd,dt,dx,resolution,filename,folder,annotations=False,color=True):
    ...
    #Set range of wavenumbers to explore (k Delta x in [0,2pi])
    k = np.linspace(0.,2.*np.pi/dx,100)
    k = k[:-1] + 0.5*(k[1]-k[0])
    
    dat_filename = os.path.join(sys.path[0], "data_nonlin_{}res_{}.dat".format(resolution,filename))
    ...
    if os.path.isfile( dat_filename ):
        ...
        for i in xrange(len(stability)):
            ...
            for j in xrange(len(stability[0])):
                ...
                dict1_file.write(dict1 % (h0,h1,u0[j],u1[j], h0,h1,0.9*u0[j],0.9*u1[j]))
                dict1_file.close()
                
                dict2_file = open(dict2_filename, "wb")
                dict2_file.write(dict2 % (g))
                dict2_file.close()
                
                os.system( os.path.join(folder, "allRun.sh") )
                
                delta_e = np.fromfile( os.path.join(folder, "energy.dat"))#, dtype=string )
                for k in delta_e:
                    print delta_e[k]
                print len(delta_e), type(delta_e[0])
                if float(delta_e[-1][3]) > 0.:
                    stability[i][j] = 0.
                else:
                    stability[i][j] = 1.
...
def main():
    ...
    folder = os.path.join(sys.path[0],"perturbedStates")
    ...
    for c in c0:
        for h in h0:
            for d in Cd:
                ...
                # stability_space(h,1.-h,c,d,dt,dx,resolution,filename,annotations=False,color=True)
